...
function Home(props) {
  ...
  if (props.cartproducts.length > 0) {
    totalcount = props.cartproducts.reduce(
      ...
      (storeCount, cartItem) => storeCount + cartItem.count,
  ...
  if (props.users.length > 0) {
    nav = (
      <NavDropdown
        title={"Hi " + props.users[0].username}
  ...
  return (
    <div>
      ...
      <Navbar collapseOnSelect expand="lg" bg="dark" variant="dark">
        <Container>
          ...
          <Navbar.Collapse id="responsive-navbar-nav">
            <Nav className="me-auto">
              ...
              {props.categoryList.map((single) => (
                <Link
                  className="nav-link"
                  to={"/products2?categoryId=" + single.id}
                >
                  {single.title}
                </Link>
              ))}
          ...
          <Link to="/cart" className="position-relative">
            ...
            <p className="cartlength">{totalcount}</p>
          </Link>
        </Container>
      </Navbar>
      {/* <h1>Home</h1> */}
      ...
      <Route
        path="/ordersummary"
        component={() => <OrderSummary toggle={drawerToggleClickHandler} />}
      />
...
export default connect(mapStateToProps, {
  loadAll: loadcontentfromactions,
  logOut: logOutfromAction,
})(Home);
